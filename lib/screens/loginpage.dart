...
import 'package:buvapp/services/services.dart';
    ...
    return Column(
      children: [
        ...
        _buildLoginWithOtherButton(
            'Se connecter avec Google', FontAwesomeIcons.google, Color.fromARGB(255, 247, 188, 184), _loginWithGoogleButtonFunction),
        _buildSpace(size),
  ...
  void _loginFunction() {
    _authService.signIn(_emailController.text, _passwordController.text).then((value) {
      ...
      if (error.code.contains('invalid-email')) {
        _buildErrorMessage("e-mail invalide");
      }
      if (error.code.contains('user-not-found')) {
        _buildErrorMessage("utilisateur introuvable");
      }
      if (error.code.contains('wrong-password')) {
        _buildErrorMessage("mot de passe incorrect");
      }
